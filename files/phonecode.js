...
(function($) {
    $.widget('custom.phonecode', {
        ...
        _create: function() {
            ...
            this.element.css({'padding-left': '67px'});
        },

        _loadData : function() {
            var self = this;
            if(!countryCache && !countryRequesting) {
                countryRequesting = $.getJSON('/assets/template/franchize/js/countries.json?v=1.1', {})
                    .done(function(json) {
                        self.data = json;
        ...
        _initSelector: function() {
            ...
            var searchLabel = $('<label class="country-phone-search-label">Р’РІРµРґРёС‚Рµ СЃС‚СЂР°РЅСѓ</label>');
        ...
        setElementSelected: function(el) {
            ...
            this.element.css({'padding-left': $(this.selector).find('.country-phone-selected').width()+33 + 'px'});
